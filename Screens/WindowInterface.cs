	...
	public class WindowInterface
	{
		...
		public virtual bool MouseHover(int x, int y, float frameDeltaTime)
		{
			if (_moving)
			{
				...
				if (_xPos + _windowWidth > _gameMain.ScreenSize.X)
				{
					_xPos = _gameMain.ScreenSize.X - _windowWidth;
				}
				if (_yPos + _windowHeight > _gameMain.ScreenSize.Y)
				{
					_yPos = _gameMain.ScreenSize.Y - _windowHeight;
				}
				if (_xPos < 0)
				{
					_xPos = 0;
